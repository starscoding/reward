...
public class GenericsUtils {
	 ...
	 * @return
	 */
	public static Object applyBeanFields(Object o1, Object o2,
                                         String... excludes) {
		...
		for (Method m : clz1.getMethods()) {
			String[] spliteName = JavaJsonUtil.getFieldName(m.getName());
			if (!"getClass".equals(m.getName())
					&& !excludesSet.contains(spliteName[1])
					&& StringUtils.isNotBlank(spliteName[1])) {
				try {
					Object val = m.invoke(o1, ea);
					if (val != null) {
						Method setMethod = clz2.getMethod(JavaJsonUtil
								.getMethodName(spliteName[1], "set"), m
								.getReturnType());
						setMethod.invoke(o2, val);
					}
					...
					System.out.println(spliteName[0]);
					System.out.println(spliteName[1]);
					e.printStackTrace();
